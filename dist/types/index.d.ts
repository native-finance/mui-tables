...
export declare type MUIDataObj = {
    [k: string]: any;
};
...
export declare type Row<R extends MUIDataObj = MUIDataObj> = Cell<R>[];
export declare type DisplayRow<R extends MUIDataObj = MUIDataObj> = {
    cells: Cell<R>[];
    rowId?: string;
    rowIds?: string[];
};
declare type CalculateCell<R extends MUIDataObj> = (entry: R) => Omit<Cell<R>, 'column'>;
export declare type SummaryFormats = 'float' | 'integer' | 'seconds' | 'hours' | 'minutes';
declare type SummaryTypes = 'AVG' | 'SUM';
...
export declare type FilterTypes = 'checkbox' | 'dropdown' | 'multiselect';
...
declare type RequiredColProps = 'name' | 'calculateCellDefinition';
export interface ColumnProp<R extends MUIDataObj> extends OptionalExceptFor<StateColumn<R>, RequiredColProps> {
}
declare type ColTypes = 'dimension' | 'metric';
...
declare type GenerateRelated<R extends MUIDataObj> = (col: ColumnProp<R>, entry: R) => ColumnProp<R>[] | null;
...
interface RowOptions<R extends MUIDataObj> {
    ...
    selectable: boolean;
    summaryTop: boolean;
    setRowProps?: (row: Row<R>, rowIndex: number) => {
        [k: string]: any;
    };
...
declare type SortColumns<R extends MUIDataObj> = (cols: StateColumn<R>[]) => StateColumn<R>[];
...
export interface HookOptions<R extends MUIDataObj> {
    ...
    onRowsSelect?: (newSelections: Row<R>[], removedSelections: Row<R>[], currentSelections: Row<R>[]) => void;
...
export interface MUITableContext<R extends MUIDataObj = MUIDataObj> extends State<R>, ContextActions<R> {
    options: Options<R>;
    rows: Row<R>[];
    displayRows: Row<R>[];
    columns: StateColumn<R>[];
